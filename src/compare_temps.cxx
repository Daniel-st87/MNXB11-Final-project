...
void filterByYear(std::vector<double>& years, std::vector<double>& temps, int startYear) {
...
int compare_temps() {
  ...
  auto dataCity1 = filterDate(csvCity1, targetDay);  // does so dataCity1 only has the info
                                                     // about lund from christmas
  ...
  filterByYear(years1, temps1, startYear);
  filterByYear(years2, temps2, startYear);

  std::vector<double> years, diffTemps;       // creates two new empty vectors, years is the years that they
                                              // share, and diffTemps is difference in temperature
  for (size_t i = 0; i < years1.size(); ++i)  // loops over all years in lunds dataset
  {
    double year = years1[i];
    auto find_year = std::find(years2.begin(), years2.end(), year);  // tries to find if year is in the year2
                                                                     // vector, if not will return year2.end()
    if (find_year != years2.end()) {
      size_t j = std::distance(years2.begin(), find_year);  // This does so we know which element of the years2 vector
                                                            // the year is in, needed when finding the difference
  ...
  grDiff->SetTitle(Form("Temperature Difference (Lund - Lulea) on Christmas (from %d)", static_cast<int>(years.front())));  // This last part does so title has
                                                                                                                            // year that the data starts from
